...
public class Ex2ArrayMethods {
    ...
    int[] generateDistribution(int nElem, double posDis, double negDis ){
        int[] arr = new int[nElem];
        int negTimes = (int)Math.round(negDis*nElem);
        int posTimes = (int)Math.round((negDis*nElem)+(posDis*nElem));
        for(int i = 0; i < negTimes; i++){
            arr[i] = -1;
        }
        for(int i = negTimes; i < posTimes; i++){
            arr[i] = 1;
        }
        for(int i = (posTimes+negTimes); i < arr.length; i++){
            arr[i] = 0;
        }
        return arr;
    }
    ...
    int[] shuffle(int[]arr){
        for(int i = 0; i < arr.length; i++){
            int post1 = rand.nextInt(arr.length);
            int post2 = rand.nextInt(arr.length);
            int value1 = arr[post1];
            int value2 = arr[post2];
            arr[post1] = value2;
            arr[post2] = value1;
        }
        return  arr;
